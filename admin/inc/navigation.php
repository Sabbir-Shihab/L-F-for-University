...
<ul class="sidebar-nav" id="sidebar-nav">
  ...
  <li class="nav-item">
    <a class="nav-link <?= $page != 'pending' ? 'collapsed' : '' ?>" href="<?= base_url."admin?page=pending" ?>">
      <i class="bi bi-clipboard-check"></i>
      <span>Pending Requests</span>
      <?php 
      $pending_total = 0;
      $piq = $conn->query("SELECT id FROM `item_list` where `status` = 0");
      if($piq) $pending_total += $piq->num_rows;
      $pcq = $conn->query("SELECT id FROM `claim_requests` where `status` = 0");
      if($pcq) $pending_total += $pcq->num_rows;
      ?>
      <?php if($pending_total > 0): ?>
        <span class="badge rounded-pill bg-danger text-light ms-4"><?= format_num($pending_total) ?></span>
      <?php endif; ?>
    </a>
  </li>
  <li class="nav-item">
  ...
  <li class="nav-item">
    ...
    <ul id="items-nav" class="nav-content collapse <?= in_array($page, ['items', 'items/manage_item', 'items/view_item']) ? 'show' : '' ?> " data-bs-parent="#sidebar-nav">
      ...
      <li>
        <a href="<?= base_url.'admin/?page=items&status=pending' ?>">
          <i class="bi bi-hourglass-split"></i><span>Pending Approval</span>
          <?php if($pitem > 0): ?>
            <span class="badge rounded-pill bg-danger text-light ms-2"><?= format_num($pitem) ?></span>
          <?php endif; ?>
        </a>
      </li>
      <li>
        <a href="<?= base_url.'admin/?page=rechived' ?>" class="<?= $page == 'rechived' ? 'active' : '' ?>">
          <i class="bi bi-circle"></i><span>Rechived</span>
        </a>
      </li>
    </ul>
  </li>
  <li class="nav-item">
    <a class="nav-link <?= $page != 'claims' ? 'collapsed' : '' ?> nav-claims" href="<?= base_url."admin?page=claims" ?>">
      <i class="bi bi-file-earmark-person"></i>
      <span>Claims</span>
      <?php 
      $pclaims = 0;
      $cq = $conn->query("SELECT * FROM `claim_requests` where `status` = 0");
      if($cq) $pclaims = $cq->num_rows;
      ?>
      <?php if($pclaims > 0): ?>
        <span class="badge rounded-pill bg-danger text-light ms-4"><?= format_num($pclaims) ?></span>
      <?php endif; ?>
    </a>
  </li>
  <li class="nav-item">
