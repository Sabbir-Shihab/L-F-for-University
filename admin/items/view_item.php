	...
	<div class="col-lg-10 col-md-12 col-sm-12 col-xs-12">
		<div class="card rounded-0">
                ...
                <div class="container-fluid mt-4">
					...
					<div class="approval-review-box">
						<div>
							...
							<p class="mb-0">Review the item details below before publishing it to the public listing.</p>
						</div>
						<div class="approval-review-actions">
							<button class="btn btn-success" type="button" id="approve_data"><i class="bi bi-check2-circle"></i> Approve &amp; Publish</button>
							<button class="btn btn-outline-danger" type="button" id="reject_data_top"><i class="bi bi-x-circle"></i> Reject</button>
						</div>
					</div>
					<?php endif; ?>
                    <dl>
			...
			<div class="card-footer py-1 text-center">
				<?php if($status == 0): ?>
					<button class="btn btn-success btn-sm bg-gradient-success rounded-0" type="button" id="approve_data_footer"><i class="fa fa-check"></i> Approve &amp; Publish</button>
					<button class="btn btn-warning btn-sm rounded-0" type="button" id="reject_data"><i class="bi bi-x-circle"></i> Reject</button>
				<?php endif; ?>
				...
				<a class="btn btn-warning btn-sm rounded-0" href="./?page=pending"><i class="bi bi-clipboard-check"></i> Pending Requests</a>
...
<script>
	$(function(){
		...
		$(document).on('click','#approve_data, #approve_data_footer',function(){
			_conf("Approve this item and make it visible to users?","approve_item", ["<?= isset($id) ? $id :'' ?>"])
		})
		$(document).on('click','#reject_data, #reject_data_top',function(){
			_conf("Reject this item request? It will not be visible to users.","reject_item", ["<?= isset($id) ? $id :'' ?>"])
		})
	...
	function reject_item($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=reject_item",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
				console.log(err)
				alert_toast("An error occurred while rejecting item.",'error');
				end_loader();
			},
			success:function(resp){
				if(typeof resp== 'object' && resp.status == 'success'){
					alert_toast(resp.msg || "Item rejected successfully.",'success');
					setTimeout(()=>{location.replace("./?page=pending");},1000);
				}else{
					var msg = (resp.msg) ? resp.msg : "Failed to reject item";
					alert_toast(msg,'error');
					end_loader();
				}
			}
