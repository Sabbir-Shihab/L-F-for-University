...
<script>
	...
	$('#manage-user').submit(function(e){
		...
		$.ajax({
			...
			error:function(err){
				console.log(err)
				$('#msg').html('<div class="alert alert-danger">An error occurred while saving user details.</div>')
				end_loader()
			},
			success:function(resp){
				if(resp.status == 'success'){
					...
					$('#msg').html('<div class="alert alert-danger">'+(resp.msg || 'Failed to save user details.')+'</div>')
					end_loader()
				}
			}
		})
