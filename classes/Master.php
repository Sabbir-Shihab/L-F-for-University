...
Class Master extends DBConnection {
	...
	function delete_img(){
		extract($_POST);
		if(empty($path)){
			return json_encode(['status' => 'failed', 'error' => 'Image path is required.']);
		}
		$real_path = realpath($path);
		$allowed_roots = array_filter([
			realpath(base_app.'uploads/banner'),
			realpath(base_app.'uploads/welcome_slider')
		]);
		$is_allowed = false;
		foreach($allowed_roots as $allowed_root){
			if($real_path && ($real_path === $allowed_root || strpos($real_path, $allowed_root.DIRECTORY_SEPARATOR) === 0)){
				$is_allowed = true;
				break;
			}
		}
		if(!$real_path || !$is_allowed){
			return json_encode(['status' => 'failed', 'error' => 'Invalid image path.']);
		}
		if(is_file($real_path)){
			if(unlink($real_path)){
				...
				$resp['status'] = 'failed';
				$resp['error'] = 'failed to delete '.$real_path;
			}
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = 'Unknown image path';
		}
		return json_encode($resp);
	}
	...
	function delete_category(){
		...
		function reject_item(){
			...
			$update = $this->conn->query("UPDATE `item_list` set `status` = 3 where id = '{$id}'");
			if($update){
				$resp['status']='success';
				$resp['msg']='Item has been rejected.';
				$this->settings->set_flashdata('success',$resp['msg']);
			}else{
				$resp['status']='failed';
				$resp['msg']='Failed to reject item: '.$this->conn->error;
			}
			return json_encode($resp);
		}
	...
	function save_page(){
		...
		if(isset($_FILES['welcome_slider']) && isset($_FILES['welcome_slider']['tmp_name'])){
			$files = $_FILES['welcome_slider'];
			$has_upload = false;
			...
			if($has_upload){
				$upload_dir = base_app.'uploads/welcome_slider/';
				...
				foreach($files['tmp_name'] as $k => $tmp_name){
					...
					if($files['error'][$k] !== UPLOAD_ERR_OK){
						return json_encode(['status' => 'failed', 'msg' => 'One of the slider images failed to upload.']);
					}
					$type = mime_content_type($tmp_name);
					if(!isset($allowed[$type]) || !@getimagesize($tmp_name)){
						return json_encode(['status' => 'failed', 'msg' => 'Only JPG, PNG, and WEBP slider images are allowed.']);
					}
					$fname = 'slide_'.($saved + 1).'.'.$allowed[$type];
					if(!move_uploaded_file($tmp_name, $upload_dir.$fname)){
						return json_encode(['status' => 'failed', 'msg' => 'Failed to save slider image.']);
					}
					$saved++;
				}
			}
