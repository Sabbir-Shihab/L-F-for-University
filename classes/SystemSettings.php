...
class SystemSettings extends DBConnection{
	...
	private function upload_error_message($code){
		switch($code){
			...
			case UPLOAD_ERR_FORM_SIZE:
				return "Uploaded image is too large. Please upload an image below the server upload limit.";
			case UPLOAD_ERR_PARTIAL:
				return "Uploaded image was only partially uploaded. Please try again.";
			case UPLOAD_ERR_NO_TMP_DIR:
				return "Server upload temporary folder is missing.";
			case UPLOAD_ERR_CANT_WRITE:
				return "Server failed to write the uploaded image.";
			case UPLOAD_ERR_EXTENSION:
				return "A PHP extension blocked the uploaded image.";
			default:
	...
	function update_settings_info(){
		...
		foreach(['img' => 'System Logo', 'cover' => 'Website Cover'] as $field => $label){
			if(isset($_FILES[$field]) && $_FILES[$field]['error'] != UPLOAD_ERR_OK && $_FILES[$field]['error'] != UPLOAD_ERR_NO_FILE){
				return json_encode([
					'status' => 'failed',
					'msg' => $label.': '.$this->upload_error_message($_FILES[$field]['error'])
				]);
			}
		}
		if(isset($_FILES['banners']) && isset($_FILES['banners']['error'])){
			foreach($_FILES['banners']['error'] as $k => $err_code){
				if($err_code != UPLOAD_ERR_OK && $err_code != UPLOAD_ERR_NO_FILE){
					return json_encode([
						'status' => 'failed',
						'msg' => 'Banner Images: '.$this->upload_error_message($err_code)
					]);
				}
			}
		}
		if(isset($_FILES['welcome_slider']) && isset($_FILES['welcome_slider']['error'])){
			foreach($_FILES['welcome_slider']['error'] as $k => $err_code){
				if($err_code != UPLOAD_ERR_OK && $err_code != UPLOAD_ERR_NO_FILE){
					return json_encode([
						'status' => 'failed',
						'msg' => 'Welcome Slider Images: '.$this->upload_error_message($err_code)
					]);
				}
			}
		}
		...
		if(isset($_FILES['welcome_slider']) && count($_FILES['welcome_slider']['tmp_name']) > 0){
			$has_upload = false;
			foreach($_FILES['welcome_slider']['tmp_name'] as $tmp_name){
				if(!empty($tmp_name)){
					$has_upload = true;
					break;
				}
			...
			if($has_upload){
				$slider_path = "uploads/welcome_slider/";
				if(!is_dir(base_app.$slider_path)){
					mkdir(base_app.$slider_path, 0777, true);
				}
				foreach(scandir(base_app.$slider_path) as $file){
					if(in_array($file, ['.', '..']))
						continue;
					if(is_file(base_app.$slider_path.$file))
						unlink(base_app.$slider_path.$file);
				}
				$allowed = ['image/jpeg' => 'jpg', 'image/png' => 'png', 'image/webp' => 'webp'];
				$saved = 0;
				foreach($_FILES['welcome_slider']['tmp_name'] as $k => $tmp_name){
					if($saved >= 5)
						break;
					if(empty($tmp_name))
						continue;
					$type = mime_content_type($tmp_name);
					if(!isset($allowed[$type]) || !@getimagesize($tmp_name)){
						return json_encode(['status' => 'failed', 'msg' => 'Welcome Slider Images: only JPG, PNG, and WEBP files are allowed.']);
					}
					$fname = 'slide_'.($saved + 1).'.'.$allowed[$type];
					if(!move_uploaded_file($tmp_name, base_app.$slider_path.$fname)){
						return json_encode(['status' => 'failed', 'msg' => 'Failed to save welcome slider image.']);
					}
					$saved++;
				}
			}
